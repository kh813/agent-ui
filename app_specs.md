...
* **対話プロンプトのGUI化 (インライン・インタラクティブカード)**:
  ...
  * **アグレッシブなゴミ文字・メタデータの排除 (新設)**: CLIが吐き出すメールアドレス、ローカルディレクトリパス、セッションの境界区切り線、およびインラインに混入する `W4;` や `4; q` といった制御残骸を、正規表現および行単位フィルタリングによりチャット側（左側）で徹底排除し、AIの純粋な答えのみを描画する。また、ストリームチャンクの途中でANSIコードが分断されてゴミ文字化するのを防ぐため、30msの蓄積タイマーで生データを完全結合してから一括でデコード・クリーンアップする結合バッファ方式を採用する。
