...
**【v5 更新概要】**
- **Claude Codeのサポート除外**: Claude Codeは有償契約時にClaude Desktop経由でGUI実行できるため、初期サポート対象から完全に除外（将来対応／ロードマップへ移動）。初期スコープは **Antigravity CLI (`agy`) のみ**に統一。
- 仕様書内の初期実装スコープに関わる記述、およびToDoリストから `claude` (Claude Code) を除外・将来対応扱いに更新。

---

# 要件定義・基本仕様書：マルチAI CLIラッパーデスクトップアプリ（仮称：Universal AI Desktop）
...
ユーザーが「黒い画面」を一切意識しなくて済むよう、Claude Desktopに酷似した2ペイン（または3ペイン）の構造とする。
...
* **対話プロンプトのGUI化 (インライン・インタラクティブカード)**:
  * **インライン表示**: `y/n` などの対話プロンプトを検知した際、入力欄の横だけでなく、チャットログの流れ（メッセージのすぐ下）に直接「選択ボタン」をインライン埋め込み表示する。
...
* **インストール先の方針（重要）**:
  * 本アプリはインストール先ディレクトリを独自に強制しない。各社の公式インストーラの既定インストール先（下表）は、いずれも**ユーザーのホームディレクトリ配下・管理者権限不要**であるため、そのまま利用する。
  * 例外的に、将来追加するCLIの公式インストール方法が管理者権限を要求するものしか提供していない場合は、その旨をUIで明示し、「アプリ内での自動インストール」ボタンは無効化して「公式サイトを開く（手動インストール）」のみを案内する。

  | エージェント | 既定インストール先 | 備考 |
  | :--- | :--- | :--- |
  | `agy`（Antigravity CLI） | macOS/Linux: `~/.local/bin`<br>Windows: `%LOCALAPPDATA%\Antigravity\` | 管理者権限不要 |
